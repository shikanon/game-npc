...
### NPC聊天接口
接口路径(URL)：/api/npc/streaming-chat
请求方式：POST
...
| 字段名称 | 数据类型 | 是否必须 | 默认值 | 描述 |
|---------|----------|----------|--------|------------------|
| code    | int  | 是 | 0 | 响应状态码，0表示正常返回消息,1表示正常结束,-1表示错误 |
| msg     | str  | 是 | "消息" | 返回的消息说明,`{code: 0, msg: "消息信息"}`, `{code: 1, msg: "chat 正常结束"}`,`msg: "{code: 1, msg: "具体错误信息"}"`|
| data    | data  | 否 | None | 返回的具体数据内容 |
...
| message | str | 是 | 无 | 存储具体的消息内容 |
| message_type | str | 是 | "text" | 消息类型，此处为文本 |
| is_finished | bool | 是 | 否 | 确认当前消息是否完结 |
...
|---------|----------|----------|--------|---------------------------------|
| name | str | 是 | 无 | 剧情名称 |
| npc_id | str | 是 | 无 | 关联角色ID |
| description | str | 是 | 无 | 剧情简短描述 |
| status | int | 是 | 无 | 状态，0: Unknown 未知, 1: Save 待发布, 2: Publish 已发布 |
| bg_image | file | 是 | 无 | 背景图片URL |
| cover_image | file | 是 | 无 | 封面图片URL |
| content_file | file | 是 | 无 | 剧情内容文件 |

返回参数：
| 字段名称 | 数据类型 | 是否必须 | 默认值 | 描述 |
|---------|----------|----------|--------|--------------------------------|
| code | int | 是 | 无 | 响应状态码，0表示执行成功 |
| msg | str | 是 | 无 | 返回的消息说明，表明剧情已成功创建 |
| data | plot | 否 | None | 返回的具体数据内容，是一个包含剧情所有信息的字典 |

---------------------------------------------------------------------------------------

### 更新剧情
接口路径(URL)：/api/plot/update
请求方式：POST
...
|---------|----------|----------|--------|--------------------------------|
| code | int | 是 | 无 | 响应状态码，0表示执行成功 |
| msg | str | 是 | 无 | 返回的消息说明，表明剧情已成功更新 |
| data | plot | 否 | None | 返回的具体数据内容，是一个包含剧情所有信息的字典 |

---------------------------------------------------------------------------------------

### 查询剧情列表
接口路径(URL)：/api/plot/query
请求方式：POST
...
|---------|----------|----------|--------|--------------------------------------------------|
| total | int | 是 | 0 | 返回的数据总量 |
| list | []plot | 是 | None | 返回的数据列表 |

---------------------------------------------------------------------------------------

### 删除剧情
接口路径(URL)：/api/plot/remove
请求方式：POST
...
|   列名   |   数据类型                  |  主键  | 默认值  | 唯一 | 外键 | 关系 |     描述     |
|---------|----------------------|--------|--------|------|----|----|-----------|
| id      | String(255)          |  是    |uuid.uuid4| 是  |    |    | 剧本ID，自增id |
| name    | String(255)           |  否    | None   | 否   |    |    | 剧本名称  |
| npc_id     | String(255)               |  否    | None   | 否   |    |    | 角色ID |
| cover_url   | String(255)            |  否    | None   | 否   |    |    | 封面URL   |
| bg_url   | String(255)               |  否    | None   | 否   |    |    | 背景URL    |
| status| Integer           |  否    | None   | 否   |    |    | 剧本状态  |
| description   | Text              |  否    | None   | 否   |    |    | 剧本简短描述    |
| content   | Text              |  否    | None   | 否   |    |    | 剧本内容    |
| created_at | DateTime          |  否    | datetime.now| 否 |  |  | 创建时间  |
| updated_at | DateTime          |  否    | datetime.now| 否 |  |  | 更新时间  |
